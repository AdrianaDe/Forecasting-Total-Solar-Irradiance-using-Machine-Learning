import sys
import os
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '../General_Code/')))
...
savingpath = path = os.path.join(os.path.dirname(__file__), '../Results/GRU/')  # CHANGE SAVINGPATH
...
################# TUNING ############################
#####################################################
find_best_hyperparameters(hyperparameters, names, GRU, start_year, n_pred, n_iterations, optimizing_i=0, savingpath=savingpath+'imf0/', scaler=scaler, method='data_all_model_random', data=imf0)  # method = 'random'

find_best_hyperparameters(hyperparameters, names, GRU, start_year, n_pred, n_iterations, optimizing_i=0, savingpath=savingpath+'imf1/', scaler=scaler, method='data_all_model_random', data=imf1) # method = 'random'
